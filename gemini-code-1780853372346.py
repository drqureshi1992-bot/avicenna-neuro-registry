...
# 1. STANDARDIZE SERVER ENVIRONMENT DEPENDENCIES (FORCES AUTOMATIC INSTALLS)
try:
    import plotly
    import xlsxwriter
except ModuleNotFoundError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "plotly", "xlsxwriter"])
...
st.set_page_config(page_title="Avicenna Neurosurgery Registry", layout="wide", page_icon="🧠")

# 3. CSS PYTHON-3.14 COMPATIBLE HOISTING
custom_css = """
<style>
.hospital-header {
...
"""
st.markdown(hospital_html, unsafe_allowed_html=True)

# 5. INITIALIZE STATE ENGINE
...
# 6. REGISTRY COUNT FILTERS LOGIC
...
st.download_button("📥 Download Filtered Summary Report", data=exp_csv, file_name="AMCH_Filtered_Summary_Report.csv", mime="text/csv")
...
if len(df_live) > 0:
    ...
    with act2:
        ...
        if uploaded_scan is not None:
            ...
            st.success("Successfully attached file details to Patient Case.")
    with act3:
